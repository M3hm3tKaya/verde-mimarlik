...
export default function SectionHeading({ label, title, description, center = false }: SectionHeadingProps) {
  return (
    <FadeIn className={center ? "text-center" : ""}>
      ...
      <h2 className="text-3xl md:text-4xl lg:text-5xl font-light text-verde-dark leading-tight">{title}</h2>
      {description && (
        <p className="text-verde-muted mt-4 max-w-2xl leading-relaxed text-base md:text-lg">
          {description}
        </p>
      )}
    </FadeIn>
