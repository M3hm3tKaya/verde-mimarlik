...
export default function HomePage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative h-screen flex items-center justify-center overflow-hidden -mt-16 sm:-mt-20">
        {/* Background placeholder */}
        <div className="absolute inset-0 bg-gradient-to-br from-verde-dark via-verde-green to-verde-dark" />
        <div className="absolute inset-0 bg-black/30" />

        {/* Decorative elements */}
        <div className="absolute top-10 right-6 sm:top-20 sm:right-20 w-32 sm:w-64 h-32 sm:h-64 rounded-full border border-verde-accent/20 opacity-40" />
        <div className="absolute bottom-20 left-4 sm:bottom-32 sm:left-16 w-48 sm:w-96 h-48 sm:h-96 rounded-full border border-verde-accent/10 opacity-30" />

        <div className="relative z-10 text-center px-4 sm:px-6">
          <FadeIn>
            <h1 className="text-5xl sm:text-7xl md:text-9xl font-light tracking-[0.15em] sm:tracking-[0.2em] text-white mb-2">
              VERDE
            </h1>
          </FadeIn>
          <FadeIn delay={200}>
            <p className="text-lg sm:text-xl md:text-2xl tracking-[0.3em] sm:tracking-[0.4em] text-verde-accent font-light">
              MİMARLIK
            </p>
          </FadeIn>
          <FadeIn delay={400}>
            <p className="mt-6 sm:mt-8 text-white/70 max-w-lg mx-auto text-sm sm:text-base md:text-lg leading-relaxed">
          ...
          <FadeIn delay={600}>
            <Link
              href="/projeler"
              className="inline-block mt-8 sm:mt-10 px-6 sm:px-8 py-3 min-h-[44px] border border-verde-accent text-verde-accent text-sm tracking-widest hover:bg-verde-accent hover:text-white transition-all duration-500"
            >
              PROJELERİMİZ
            </Link>
          </FadeIn>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {featuredProjects.map((project, i) => (
              <FadeIn key={project.id} delay={i * 150}>
                <Link href={`/projeler/${project.slug}`} className="group block">
                  <div className="relative w-full aspect-[4/3] sm:aspect-[16/9] md:aspect-[21/9] overflow-hidden bg-verde-light/10">
                    <div
                    ...
                    {/* Hover overlay */}
                    <div className="absolute inset-0 bg-verde-dark/70 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-4 sm:p-8 md:p-12">
                      <div>
                        <span className="text-xs tracking-widest text-verde-accent uppercase">
                          {project.category}
                        </span>
                        <h3 className="text-lg sm:text-2xl md:text-3xl font-light text-white mt-2">{project.title}</h3>
          ...
          <FadeIn delay={300}>
            <div className="mt-12 text-center">
              <Link
                href="/projeler"
                className="inline-block px-6 sm:px-8 py-3 min-h-[44px] border border-verde-dark text-verde-dark text-sm tracking-widest hover:bg-verde-dark hover:text-white transition-all duration-500"
              >
                TÜM PROJELER
              </Link>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {stats.map((stat, i) => (
            <FadeIn key={stat.label} delay={i * 100}>
              <div className="text-center">
                <div className="text-3xl sm:text-4xl md:text-5xl font-light text-verde-dark">{stat.value}</div>
          ...
          <div className="mt-10 sm:mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {services.map((service, i) => (
              <FadeIn key={service.id} delay={i * 100}>
                <div className="bg-white/80 p-6 sm:p-8 hover:shadow-lg transition-shadow duration-500 group">
      ...
      <section className="py-16 sm:py-24 lg:py-32 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <FadeIn>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-light text-verde-dark leading-tight">
              Projenizi Birlikte<br />Hayata Geçirelim
            </h2>
            <p className="text-verde-muted mt-4 sm:mt-6 max-w-lg mx-auto leading-relaxed text-sm sm:text-base">
